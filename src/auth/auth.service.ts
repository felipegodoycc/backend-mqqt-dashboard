...
export class AuthService {
    ...
    async validateUserByPassword(loginAttempt: LoginDTO) {
        ...
        const userToAttempt = await this.userService.findByUsername(loginAttempt.username);
        const user = await this.userService.getUser(userToAttempt._id);

        return new Promise(async (resolve, reject) => {
            const loginPassword = loginAttempt.password;
            if (user.active === false) { return reject('Usuario deshabilitado, no puede iniciar sesion'); }
            if (user.checkPassword(loginPassword)) {
                const {token} = this.createJwtPayload(userToAttempt);
                if (await this.authTokenModel.findOneAndDelete({ user: userToAttempt._id }).exec()) { 
                    console.log(`${userToAttempt.username} ya tiene token`); 
                }
